    ...
    try:
        if request.method == 'POST':
            ...
            if user_registration_form.is_valid():
                ...
                return HttpResponseRedirect(reverse(viewname='user_authentication'))
...
def user_authentication(request):
    ...
    #TODO add in re-send authentication email
    '''
    context = {
        'title' : "Music App"
    }
...
def user_authentication_success(request, user_id, token):
    ...
    try:
        ...
        one_time_token = OneTimeToken.objects.get(
            token = token
            , user_id = user_id
            , is_used = False
            , is_active = True
...
def user_logout(request):
    '''
    This view displays the logout page if they've successfully logged out.
    '''
    logout(request)
    context = {
        'title' : "Music App"
...
def check_your_email_password(request, user_id):
    ...
    #TODO display successful message to the user if they wnat the email resent.
    '''
    ...
    if request.method == 'POST':
        ...
        try:
            current_token = OneTimeToken.objects.get(
                user_id = user_id
                , is_used = False
                , is_active = True
...
def user_success_reset_password(request):
    '''
    #TODO unsure whether I need to add in tokens for this?
    '''
